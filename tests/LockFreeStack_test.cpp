...
TEST_F(LockFreeStackFixture, EmptyStack_TryPopFalse) {
    ...
    Stack st(slot_mgr, retired_mgr);

    std::cout << "\n[EmptyStack] init: " << st.debug_to_string() << std::endl;
    ...
    std::cout << "[EmptyStack] final: " << st.debug_to_string() << std::endl;
}
...
TEST_F(LockFreeStackFixture, PushThenPop_LIFOOrder) {
    ...
    std::cout << "\n[LIFO] after push 1..5: " << st.debug_to_string() << std::endl;
    ...
    EXPECT_TRUE(st.empty());
    std::cout << "[LIFO] after pop all: " << st.debug_to_string() << std::endl;
...
TEST_F(LockFreeStackFixture, DrainAll_ForceCollectEverything) {
    ...
    std::cout << "\n[DrainAll] after push 3: " << st.debug_to_string() << std::endl;
    ...
    std::cout << "[DrainAll] after drain_all: " << st.debug_to_string() << std::endl;
}
